...
import { UPDATE_QUANTITY_MUTATION } from "../../graphql/mutation";
import { GET_PRODUCTS_CART } from "../../graphql/query";
import useProtected from "../../hooks/useProtected";

const PaymentDetails = ({ paymentSum }) => {
  const [cardSelected, setCardSelected] = useState(true);

  return (
    <div className="col-md-4 mb-3 mt-5 mt-lg-0">
      <div className="payment-info">
        <div className="d-flex justify-content-between align-items-center">
          <h4>Card details</h4>
        </div>

        <span className="type d-block mt-1 mb-1">Payment method</span>
        ...
        {cardSelected ? (
          <>
            ...
            <div className="d-flex justify-content-between information">
              <span>Subtotal</span>
              <span>${paymentSum.cartSum.toFixed(2)}</span>
            </div>

            <div className="d-flex justify-content-between information">
              <span>Shipping</span>
              <span>${paymentSum.shippingSum.toFixed(2)}</span>
            </div>

            <div className="d-flex justify-content-between information">
              <span>Total(Incl. taxes)</span>
              <span>
                ${(paymentSum.cartSum + paymentSum.shippingSum).toFixed(2)}
              </span>
            </div>
          ...
          <>
            ...
            <div className="mt-3">
              <button className="btn btn-primary btn-block ">
                ...
                {paymentSum.cartSum + paymentSum.shippingSum}
              </button>
            </div>
          </>
        )}
...
const CartProductCard = ({ product, count, updateCart }) => {
  useProtected();
  const [updateQuantityFunction] = useMutation(UPDATE_QUANTITY_MUTATION);
  const { setCart, getProductsCartFunction } = updateCart;

  return (
    <div className="d-flex justify-content-between align-items-center mt-3 p-2 items rounded">
      <div className="d-flex flex-row" style={{ maxWidth: 250, width: "100%" }}>
        <img
          alt=""
        ...
        <div className="ml-2">
          ...
          <span className="spec">{product?.brand}</span>
        </div>
      </div>
      <div>
        <button
          ...
          onClick={() =>
            updateQuantityFunction({
              variables: {
                cartInput: { productId: product.id, quantity: count + 1 },
              },
            }).then(() =>
              getProductsCartFunction().then((res) =>
                setCart(res.data.getProductsCart)
              )
            )
          }
        >
...
const Cart = () => {
  ...
  const [getProductsCartFunction, { loading, error }] = useLazyQuery(
    GET_PRODUCTS_CART,
  ...
  const updateCart = {
    setCart,
    getProductsCartFunction,
  };

  useEffect(() => {
    getProductsCartFunction().then((res) => setCart(res.data.getProductsCart));
  ...
  const cartSum = cart.reduce((acc, cartItem) => {
    return acc + cartItem.quantity * cartItem.product.price;
  }, 0);

  const shippingSum = 20;
  ...
  return (
    <>
      ...
      <div className="container mt-5 p-3 rounded cart">
        <h3 className="text-muted align-self-start">Cart</h3>
        <div className="d-flex g-5 row no-gutters">
          <div className="col">
            <div className="product-details mr-2">
              <div className="d-flex flex-row align-items-center">
                <i className="fa fa-long-arrow-left"></i>
                <h5 className="ml-2">Continue Shopping</h5>
              </div>
              <hr />
          ...
          {cart.length > 0 && <PaymentDetails paymentSum={paymentSum} />}
        </div>
      </div>
      <Footer />
    </>
